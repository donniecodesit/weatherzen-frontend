...
export async function createObservation(observation, signal) {
  ...
  const options = { method: "POST", headers, body: JSON.stringify({ data: observation }), signal };
...
export async function updateObservation(observation, signal) {
  ...
  const options = { method: "PUT", headers, body: JSON.stringify({ data: observation }), signal };
  return await fetchJson(url, options, observation);
}